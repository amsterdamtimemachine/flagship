...
# Simple wrapper for main.ts with common operations
...
case $COMMAND in
  ...
  "prod"|"production")
    ...
    echo
    if [[ $REPLY =~ ^[Yy]$ ]]; then
      bun run src/main.ts --preset PRODUCTION --output ./visualization.bin
      if [ $? -eq 0 ]; then
        echo ""
  ...
  "quick")
    ...
    echo ""
    PRESET=DEVELOPMENT OUTPUT_PATH=./quick-test.bin bun run src/main.ts
    if [ $? -eq 0 ]; then
      echo ""
  ...
  "help"|*)
    echo -e "${BLUE}📖 Usage: ./generate.sh <command>${NC}"
    echo ""
    echo "Commands:"
