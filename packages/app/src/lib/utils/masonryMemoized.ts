...
/**
 * Extract feature ID from masonry item element
 */
function getFeatureId(item: HTMLElement): string | null {
  // Look for data-feature-id attribute on the masonry-item
  const featureId = item.getAttribute('data-feature-id');
  if (featureId) return featureId;
  
  // Fallback: look for feature ID in child FeatureCard
  const featureCard = item.querySelector('[data-feature-id]');
  return featureCard?.getAttribute('data-feature-id') || null;
}
