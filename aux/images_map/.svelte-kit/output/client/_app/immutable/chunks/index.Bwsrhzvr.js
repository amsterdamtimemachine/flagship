...
function z(e) {
	...
	c.reverse(), l.sort((i, a) => i.claim_order - a.claim_order);
	for (let i = 0, a = 0; i < l.length; i++) {
		for (; a < c.length && l[i].claim_order >= c[a].claim_order; ) a++;
...
function K(e, t, n) {
	...
	r && r.m(t, n),
		b(() => {
			const c = e.$$.on_mount.map(P).filter(S);
...
function $e(e, t, n, r, s, c, l = null, f = [-1]) {
	const i = L;
	N(e);
	const a = (e.$$ = {
		fragment: null,
	...
	if (
		((a.ctx = n
			? n(e, t.props || {}, (u, _, ...g) => {
					...
					return (
						a.ctx &&
							s(a.ctx[u], (a.ctx[u] = v)) &&
							(!a.skip_bound && a.bound[u] && a.bound[u](v), o && X(e, u)),
						_
					);
				})
			: []),
		...
		if (t.hydrate) {
			H();
			const u = V(t.target);
			a.fragment && a.fragment.l(u), u.forEach(R);
		} else a.fragment && a.fragment.c();
		t.intro && J(e.$$.fragment), K(e, t.target, t.anchor), M(), C();
	}
	N(i);
}
class pe {
