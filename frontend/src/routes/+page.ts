...
import type { Heatmap, HeatmapResponse } from '@atm/shared-types';
...
export const load: PageLoad = async ({ fetch }) => {
  ...
  const baseUrl = import.meta.env.MODE === 'production' ? PUBLIC_SERVER_DEV_URL : PUBLIC_SERVER_PROD_URL;
	...
	const metadataUrl = baseUrl + '/grid/metadata' //?startDate=1800-01-01&endDate=1950-01-01'	
	const metadata = await fetchApi<HeatmapResponse>(metadataUrl, fetch);
