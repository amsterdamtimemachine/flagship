<script lang="ts">
  ...
  const { elements: { root: classRoot, item: classItem }, states: { value: classValue } } = createToggleGroup({
    type: 'multiple',
    defaultValue: ['Image'] 
  });
  
  const { elements: { root: tagRoot, item: tagItem }, states: { value: tagValue } } = createToggleGroup({
    type: 'multiple',
  });
   ...
   */
  function getTagsForClass(contentClass: ContentClass, statistics: BinaryMetadata['featuresStatistics']) {
   ...
   */
  function findIntersectionTags(classes: ContentClass[], statistics: BinaryMetadata['featuresStatistics']) {
